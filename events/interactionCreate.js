...
module.exports = (client, inter) => {
    ...
    if (!command) return inter.reply({ embeds: [ new EmbedBuilder().setColor('#ff0000').setDescription('❌ | Ошибка! Пожалуйста сообщите разработчику!')], ephemeral: true, }), client.slash.delete(inter.commandName)
    ...
    if (command.voiceChannel) {
            if (!inter.member.voice.channel) return inter.reply({ embeds: [ new EmbedBuilder().setColor('#ff0000').setDescription(`❌ | Вы не в голосовом канале`)], ephemeral: true, })
